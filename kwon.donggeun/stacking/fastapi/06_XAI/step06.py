"""
Step 06: SHAP + Permutation Importance 해석

역할: FastAPI
목적: 스태킹 튜닝 모델에 대해 SHAP과 Permutation Importance를 계산해
      피처별 중요도를 두 가지 관점에서 비교한다.
      SHAP은 모델 설명이며 인과가 아님.
출력: SHAP 상위 20개, Permutation Importance 상위 20개, 패밀리별 합계
캐시: step06_shap_global.json
"""
import sys
...
router = APIRouter(prefix="/06", tags=["06. SHAP + Permutation Importance"])
...
def shap_interpretation():
    """Step 06: SHAP + Permutation Importance 피처 중요도 계산. 캐시 있으면 캐시 사용."""
    if is_done("step06"):
        cached = load_json("step06_shap_global")
        if cached:
    ...
    save_json("step06_shap_global", result)
    mark_done("step06", {"scopes": list(result["by_scope"].keys())})
