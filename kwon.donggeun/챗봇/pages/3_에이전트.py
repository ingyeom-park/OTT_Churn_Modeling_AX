...
B안: (감성/공감형, 80자 이내로 작성)"""
    ...
    sms_a, sms_b = '', ''
    import re
    match_a = re.search(r'A안[:\s]*(.*?)(?=B안|$)', raw, re.DOTALL)
    match_b = re.search(r'B안[:\s]*(.*?)$', raw, re.DOTALL)
    if match_a: sms_a = match_a.group(1).strip().strip('"')
    if match_b: sms_b = match_b.group(1).strip().strip('"')
    if not sms_a: sms_a = raw.strip()
...
def tool_save_report(n: int = 10, min_prob: float = 0.6, api_key: str = '') -> str:
    ...
    for i, (_, row) in enumerate(top_customers.iterrows()):
        ...
        rows.append({
            ...
            '문자(A안)': sms_data.get('문자A', ''),
            '문자(B안)': sms_data.get('문자B', ''),
        })
