...
if "dark_mode" not in st.session_state:
    st.session_state.dark_mode = False  # 다크모드 해제
...
• 고객을 클릭하면 맞춤 영화 추천과 메시지를 볼 수 있어요
    ...
    try:
        ...
        eligible_df     = df[df["_reg"] <= eligible_cutoff]
        eligible_prev   = df[df["_reg"] <= (eligible_cutoff - timedelta(days=1))]
...
{"⚠️ 아직 관측창(21일)을 완료한 고객이 없어요. 3월 22일부터 이탈 예측이 시작돼요." if eligible_cnt == 0 else f"현재 {eligible_cnt:,}명의 이탈 확률 계산이 가능합니다."}
        ...
        wtcol = "total_watch_time_min" if "total_watch_time_min" in df.columns else None
        genre_totals = {}
        if wtcol:
...
총 {total_items}개 장르가 시청되고 있어요. 장르 다양성은 고객 만족도와 연관이 높아요.
        ...
        if inactive_cnt > 0:
            ...
            avail    = [c for c in ["days_inactive","age_group","gender_kor","is_promotion"] if c in act_show.columns]
            show1    = act_show[avail].copy()
            show1.insert(0, "Num", range(1, len(show1)+1))
            show1.columns = [{"days_inactive":"미접속일수","age_group":"연령대",
                              "gender_kor":"성별","is_promotion":"프로모션"}.get(c,c) for c in show1.columns]
            styled1 = show1.style\
        ...
        if eligible_cnt == 0:
            ...
            show2 = hr_cum[["churn_risk","age_group","is_promotion"]].copy() if all(
                c in hr_cum.columns for c in ["age_group","is_promotion"]
            ) else hr_cum[["churn_risk"]].copy()
            show2.insert(0, "Num", range(1, len(show2)+1))
            show2["churn_risk"] = (show2["churn_risk"] * 100).round(1)
            col_map2 = {"churn_risk":"이탈확률(%)","age_group":"연령대","is_promotion":"프로모션"}
...
• 기준일을 바꾸면 두 섹션 모두 자동 업데이트돼요
    ...
    try:
        ...
        elig_w   = df_w[df_w["_reg"] <= eligible_cutoff_w]
        elig_cnt = len(elig_w)
