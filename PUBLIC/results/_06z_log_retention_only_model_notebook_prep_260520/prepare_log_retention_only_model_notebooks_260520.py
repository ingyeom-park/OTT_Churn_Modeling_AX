...
RUN_NAME = "PUBLIC_log_retention_only_model_notebook_prep_260520"
TODAY = "2026-05-20"
...
WORK_DIR = RESULTS_DIR / "_06z_log_retention_only_model_notebook_prep_260520"
ZIP_DIR = PUBLIC / "zip"
NOTE_PATH = PUBLIC / "note.md"
ZIP_PATH = ZIP_DIR / "PUBLIC_log_retention_only_model_notebook_prep_260520_review_package.zip"

SOURCE_OVERALL = DATA_DIR / "06z_expanded_dataset_log_retention.csv"
SOURCE_PROMO0 = DATA_DIR / "06z_expanded_dataset_promo_0_log_retention.csv"
SOURCE_PROMO1 = DATA_DIR / "06z_expanded_dataset_promo_1_log_retention.csv"

MODEL_INPUT_PROMO0 = DATA_DIR / "06z_model_input_promo_0_log_retention_only.csv"
MODEL_INPUT_PROMO1 = DATA_DIR / "06z_model_input_promo_1_log_retention_only.csv"
...
MODEL_SPECS = [
    {
        "key": "gb_promo0",
        "notebook": NOTEBOOK_DIR / "06z_gb_promo0_logretention_only.ipynb",
        "data": MODEL_INPUT_PROMO0,
        "out_dir": RESULTS_DIR / "_06z_log_retention_only_model_rerun_260520" / "gb_promo0",
        "promo": 0,
    ...
    {
        "key": "gb_promo1",
        "notebook": NOTEBOOK_DIR / "06z_gb_promo1_logretention_only.ipynb",
        "data": MODEL_INPUT_PROMO1,
        "out_dir": RESULTS_DIR / "_06z_log_retention_only_model_rerun_260520" / "gb_promo1",
        "promo": 1,
        "model_kind": "gradientboosting",
        "model_label": "GradientBoostingClassifier conservative",
    },
    {
        "key": "lr_promo0",
        "notebook": NOTEBOOK_DIR / "06z_lr_promo0_logretention_only.ipynb",
        "data": MODEL_INPUT_PROMO0,
        "out_dir": RESULTS_DIR / "_06z_log_retention_only_model_rerun_260520" / "lr_promo0",
        "promo": 0,
    ...
    {
        "key": "lr_promo1",
        "notebook": NOTEBOOK_DIR / "06z_lr_promo1_logretention_only.ipynb",
        "data": MODEL_INPUT_PROMO1,
        "out_dir": RESULTS_DIR / "_06z_log_retention_only_model_rerun_260520" / "lr_promo1",
        "promo": 1,
        "model_kind": "logisticregression",
        "model_label": "LogisticRegression baseline",
    },
...
def read_rows_cols(path: Path) -> tuple[int | None, int | None, str]:
    try:
        ...
        return None, None, f"FAIL: {type(exc).__name__}: {exc}"
...
def feature_role(column: str) -> tuple[str, bool, str, str]:
    ...
    if column in LOG_RETENTION:
        return "log_retention_feature", True, "", "log retention only feature"
    return "feature", True, "", ""
                ...
                rows.append(
                    {
                        ...
                        "exclude_reason": "raw retention removed from log retention only model input",
...
def manual_execution_guide() -> str:
    return """# 06z log retention only manual execution guide
...
def prep_memo(input_profile: list[dict]) -> str:
    ...
    return f"""# 06z log retention only model notebook prep memo
...
사용자 결정으로 feature set은 log retention only로 고정되었다. feature set 논의는 종료되었고, 기존 `retention_w2_ratio`, `retention_w3_ratio`는 모델 입력에서 삭제한다.
...
def append_note(input_profile: list[dict]) -> None:
    marker = "## 2026-05-20 | PUBLIC_log_retention_only_model_notebook_prep_260520"
...
def create_final_checks(
    ...
    add("log_retention_only_input_created_promo0", "PASS" if MODEL_INPUT_PROMO0.exists() else "FAIL", "exists", str(MODEL_INPUT_PROMO0.exists()), "critical")
    add("log_retention_only_input_created_promo1", "PASS" if MODEL_INPUT_PROMO1.exists() else "FAIL", "exists", str(MODEL_INPUT_PROMO1.exists()), "critical")
    ...
    add("input_inventory_created", "PASS", "exists", str((WORK_DIR / "06z_log_retention_input_inventory.csv").exists()), "critical")
    add("input_profile_created", "PASS", "exists", str((WORK_DIR / "06z_log_retention_input_profile.csv").exists()), "critical")
    add("feature_manifest_created", "PASS", "exists", str((WORK_DIR / "06z_log_retention_only_feature_manifest.csv").exists()), "critical")
    add("static_checks_created", "PASS", "exists", str((WORK_DIR / "06z_logretention_only_notebook_static_checks.csv").exists()), "critical")
    add("manual_execution_guide_created", "PASS", "exists", str((WORK_DIR / "06z_logretention_only_manual_execution_guide.md").exists()), "critical")
    add("prep_memo_created", "PASS", "exists", str((WORK_DIR / "06z_logretention_only_model_notebook_prep_memo.md").exists()), "critical")
...
def make_zip(expected_files: list[Path]) -> list[dict]:
    ...
    inventory_path = WORK_DIR / "06z_logretention_only_notebook_prep_review_zip_inventory.csv"
...
def main() -> None:
    ...
    write_csv(dir_inventory, WORK_DIR / "06z_logretention_only_existing_folder_inventory.csv")

    input_inventory = create_input_inventory()
    write_csv(input_inventory, WORK_DIR / "06z_log_retention_input_inventory.csv")
    ...
    write_csv(input_profile, WORK_DIR / "06z_log_retention_input_profile.csv")

    input_validation = create_model_inputs(datasets)
    write_csv(input_validation, WORK_DIR / "06z_log_retention_only_input_validation.csv")
    if any(row["validation_status"] == "FAIL" for row in input_validation):
        raise SystemExit("Log retention only input validation failed. Stop before notebook generation.")

    feature_manifest = create_feature_manifest()
    write_csv(feature_manifest, WORK_DIR / "06z_log_retention_only_feature_manifest.csv")

    create_notebooks()
    static = static_checks()
    write_csv(static, WORK_DIR / "06z_logretention_only_notebook_static_checks.csv")

    (WORK_DIR / "06z_logretention_only_manual_execution_guide.md").write_text(manual_execution_guide(), encoding="utf-8")
    (WORK_DIR / "06z_logretention_only_model_notebook_prep_memo.md").write_text(prep_memo(input_profile), encoding="utf-8")
    append_note(input_profile)
    note_tail = "\n".join(NOTE_PATH.read_text(encoding="utf-8").splitlines()[-180:]) + "\n"
    (WORK_DIR / "note_tail_PUBLIC_log_retention_only_model_notebook_prep_260520.md").write_text(note_tail, encoding="utf-8")

    expected_zip_files = [
        WORK_DIR / "06z_log_retention_input_inventory.csv",
        WORK_DIR / "06z_log_retention_input_profile.csv",
        WORK_DIR / "06z_log_retention_only_input_validation.csv",
        WORK_DIR / "06z_log_retention_only_feature_manifest.csv",
        WORK_DIR / "06z_logretention_only_notebook_static_checks.csv",
        WORK_DIR / "06z_logretention_only_manual_execution_guide.md",
        WORK_DIR / "06z_logretention_only_model_notebook_prep_memo.md",
        WORK_DIR / "06z_logretention_only_model_notebook_prep_final_checks.csv",
        MODEL_INPUT_PROMO0,
        MODEL_INPUT_PROMO1,
        *[spec["notebook"] for spec in MODEL_SPECS],
        WORK_DIR / "note_tail_PUBLIC_log_retention_only_model_notebook_prep_260520.md",
        WORK_DIR / "06z_logretention_only_notebook_prep_review_zip_inventory.csv",
        Path(__file__).resolve(),
    ]

    zip_inventory = make_zip([path for path in expected_zip_files if path.name != "06z_logretention_only_model_notebook_prep_final_checks.csv"])
    zip_contains_inventory = any(row["file_name"] == "06z_logretention_only_notebook_prep_review_zip_inventory.csv" and row["present"] for row in zip_inventory)
    final_checks = create_final_checks(input_inventory, input_profile, input_validation, static, True, zip_contains_inventory)
    write_csv(final_checks, WORK_DIR / "06z_logretention_only_model_notebook_prep_final_checks.csv")

    final_zip_inventory = make_zip(expected_zip_files)
    zip_contains_inventory = any(row["file_name"] == "06z_logretention_only_notebook_prep_review_zip_inventory.csv" and row["present"] for row in final_zip_inventory)
    final_checks = create_final_checks(input_inventory, input_profile, input_validation, static, True, zip_contains_inventory)
    write_csv(final_checks, WORK_DIR / "06z_logretention_only_model_notebook_prep_final_checks.csv")
